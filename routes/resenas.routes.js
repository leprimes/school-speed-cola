...
router.post("/api/resenaProveedor", async (req, res) => {
  ...
  try {
    ...
    res.json({
      message: "Reseña de proveedor guardada y promedio actualizado",
      promedio,
    });
  } catch (error) {
    console.error("Error guardando reseña de proveedor:", error);
    res.status(500).json({
      error: "Error guardando reseña de proveedor",
...
router.post("/api/resenaUsuario", async (req, res) => {
  ...
  try {
    ...
    res.json({
      message: "Reseña de usuario guardada y promedio actualizado",
      promedio,
    });
  } catch (error) {
    console.error("Error guardando reseña de usuario:", error);
    res.status(500).json({
      error: "Error guardando reseña de usuario",
      details: error.message,
    });
  }
});
...
router.get("/api/resena/:tipo/:idA/:idB", async (req, res) => {
  ...
  try {
    ...
    if (tipo === "proveedor") {
      query =
        "SELECT * FROM resenaProveedor WHERE idUsuario = ? AND idProveedor = ?";
    } else if (tipo === "usuario") {
      query =
        "SELECT * FROM resenaUsuario WHERE idProveedor = ? AND idUsuario = ?";
    } else {
    ...
    res
      .status(500)
      .json({ error: "Error verificando reseña", details: error.message });
  }
});
